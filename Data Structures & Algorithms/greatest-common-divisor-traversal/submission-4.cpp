...
vector<int>calcprimfactors(int num){
    ...
    bool canTraverseAllPairs(vector<int>& nums) {
        ...
        for(int i = 0; i < n; i++){
            ...
            for(int p : factors){
                //if the primenumber existed before then unite the indices
                if(firstprimeat.count(p)){
                    if(dsu.unite(i, firstprimeat[p]));
                }
                //if not existed before, then make an entry at which the index does the primenumber exist
                else firstprimeat[p] = i;
            }
        }
        //checking if everyone has the same root as 0
        for(int i = 1; i < n;i++){
